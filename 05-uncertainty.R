...
word_list <- read_lines("lists/uncertainty.csv")
...
map_df(word_list, ~{
  ...
  mutate(ct = ifelse(ct == 0, NA, ct)) -> uncertain_df

#' ## ID to Doc Name mapping:

distinct(uncertain_df, doc_id, doc_num) %>%
  left_join(total_words) %>%
  kable()

#' ## Overall uncertain word frequency per document

#+ overall_uncertain_freq_summary
count(uncertain_df, doc_id, wt=ct) %>%
  mutate(doc_num = as.character(as.numeric(factor(doc_id)))) %>%
  left_join(total_words) %>%
  ...
  labs(
    x="Document #", y=NULL,
    title="Percent of 'uncertain' words of total words in document corpus"
  ) +
  theme_ipsum_rc(grid="Y")

#' ## General frequency per document

#+ uncertain_frequency, fig.height=8
ggplot(uncertain_df, aes(doc_num, keyword, fill=ct)) +
  geom_tile(color="#2b2b2b", size=0.125) +
  scale_x_discrete(expand=c(0,0)) +
  scale_y_discrete(expand=c(0,0)) +
  viridis::scale_fill_viridis(direction=-1, na.value="white") +
  labs(x=NULL, y=NULL, title='"Uncertainty" Frequency') +
  theme_ipsum_rc(grid="")

#' ## Normalized frequency per document

#+ uncertain_frequency_normalized, fig.height=8
left_join(uncertain_df, total_words) %>%
  ...
  scale_y_discrete(expand=c(0,0)) +
  viridis::scale_fill_viridis(direction=-1, na.value="white") +
  labs(x=NULL, y=NULL, title='"Uncertainty" Frequency (normalized)') +
  theme_ipsum_rc(grid="")
