...
word_list <- read_lines("lists/explanatory.csv")
...
map_df(word_list, ~{
  ...
  mutate(ct = ifelse(ct == 0, NA, ct)) -> explanatory_df

#' ## ID to Doc Name mapping:

distinct(explanatory_df, doc_id, doc_num) %>%
  left_join(total_words) %>%
  kable()

#' ## Overall Explanatory word frequency per document

#+ overall_explanatory_freq_summary
count(explanatory_df, doc_id, wt=ct) %>%
  mutate(doc_num = as.character(as.numeric(factor(doc_id)))) %>%
  left_join(total_words) %>%
  ...
  labs(
    x="Document #", y=NULL,
    title="Percent of 'explanatory' words of total words in document corpus"
  ) +
  theme_ipsum_rc(grid="Y")

#' ## General frequency per document

#+ explanatory_frequency, fig.height=8
ggplot(explanatory_df, aes(doc_num, keyword, fill=ct)) +
  geom_tile(color="#2b2b2b", size=0.125) +
  scale_x_discrete(expand=c(0,0)) +
  scale_y_discrete(expand=c(0,0)) +
  viridis::scale_fill_viridis(direction=-1, na.value="white") +
  labs(x=NULL, y=NULL, title='"Explanatory" Frequency') +
  theme_ipsum_rc(grid="")

#' ## Normalized frequency per document

#+ explanatory_normalized, fig.height=8
left_join(explanatory_df, total_words) %>%
  ...
  scale_y_discrete(expand=c(0,0)) +
  viridis::scale_fill_viridis(direction=-1, na.value="white") +
  labs(x=NULL, y=NULL, title='"Explanatory" Frequency (normalized)') +
  theme_ipsum_rc(grid="")
